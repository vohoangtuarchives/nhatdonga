...
<?php if ($type == 'chi-nhanh') {
	$chi_nhanh = $d->rawQuery("SELECT name$lang,map from table_news where type='chi-nhanh' and find_in_set('hienthi',status) order by numb");
	if ($chi_nhanh) { ?>
...
<?php if ($type == 'video-cbo') { ?>
	<?php $videos = $d->rawQuery("SELECT name$lang,link_video from table_photo where type='video' and find_in_set('hienthi',status) order by numb"); ?>
	<?php if (!empty($videos)) { ?>
...
<?php if ($type == 'video-slick') {
	$videos = $d->rawQuery("SELECT name$lang,link_video from table_photo where type='video' and find_in_set('hienthi',status) and find_in_set('noibat',status) order by numb");
	if (!empty($videos)) { ?>
