...
// Ensure PHPMailer is loaded (Composer or Manual)
...
class EmailService
{
    ...
    {
        // Use GlobalHelper or global
        global $configBase;
        if (!isset($configBase) && function_exists('Tuezy\config')) {
             $config = \Tuezy\config();
             $configBase = $config['database']['url']; // simplified
        }
    ...
    public function markdown($path='', $params=array())
    {
        ...
        {
            ob_start();
            // Adjust path for sample mail. Assuming 'sample' is in ROOT.
            $samplePath = defined('BASE_PATH') ? BASE_PATH . "/libraries/sample/mail/".$path.".php" : dirname(dirname(__DIR__))."/libraries/sample/mail/".$path.".php";
