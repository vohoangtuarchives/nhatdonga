...

{
    ...
    {
        // Check if transaction is active (both our flag and PDO's inTransaction)
        $pdo = $this->pdo();
        if (!$this->transaction && !$pdo->inTransaction()) {
        ...
        try {
            ...
            // If commit fails, transaction might already be closed
            $this->transaction = false;
            // Check if error is about no active transaction
            if (strpos($e->getMessage(), 'no active transaction') !== false || 
                strpos($e->getMessage(), 'There is no active transaction') !== false) {
                return true; // Transaction already closed, consider it success
            }
            throw $e;
        }
    }
     ...
     * @param int|array|null $numRows Array to define SQL limit in format Array ($count, $offset)
     ...
     * @return array|false|PDODb Contains the returned rows from the select query.
    ...
    {
        $result = $this->rawQueryOne($query, $params);
        return $result ?: null;
    }
    /**
     ...
     * @param array|null  $params Variables array to bind to the SQL statement.
...

    {
        ...
        try {
            ...
            // If rollback fails, transaction might already be closed
            $this->transaction = false;
            // Check if error is about no active transaction
            if (strpos($e->getMessage(), 'no active transaction') !== false || 
                strpos($e->getMessage(), 'There is no active transaction') !== false) {
    ...
    /**
     * Update query. Be sure to first call where() to set the update criteria.
     *
     * @param string $tableName The name of the database table to work with.
     * @param array $tableData Array of data to update the desired row.
     * @param int|array|null $numRows Limit on the number of rows to update.
     *
     * @return bool
     */
...

    {
        ...
        $msg = "
        <div style='background: #f8d7da; color: #721c24; padding: 10px; border: 1px solid #f5c6cb; border-radius: 5px; font-family: sans-serif; margin: 10px 0;'>
            <h3 style='margin-top: 0;'>Database Exception: {$name}</h3>
            " . ($errorNumber ? "<p><strong>Error Code:</strong> {$errorNumber}</p>" : "") . "
            " . ($erroMessage ? "<p><strong>Message:</strong> {$erroMessage}</p>" : "") . "
            " . ($sql ? "<div style='background: #eee; padding: 10px; margin-top: 10px; border-radius: 3px;'><pre style='margin: 0; white-space: pre-wrap;'>{$sql}</pre></div>" : "") . "
        </div>";
    ...
    public function createLogs($user, $act, $com, $id)
