...
/**
 * AddonsAPIController - Handles addons API requests
 */
class AddonsAPIController extends BaseAPIController
{
    ...
    {
        global $custom; // Still using global for now

        switch ($type) {
            case 'video-fotorama':
                $this->renderVideoFotorama();
                break;
    ...
    /**
     * Render video fotorama
     */
    private function renderVideoFotorama(): void
    {
        global $custom;
        $video_home = $this->photoRepo->getFeaturedVideos('video');
        ...
        if (count($video_home)) { ?>
            <div id="fotorama-videos" data-width="100%" data-thumbmargin="10" data-height="330" data-fit="cover" data-thumbwidth="140" data-thumbheight="80" data-allowfullscreen="true" data-nav="thumbs">
                <?php foreach ($video_home as $v) { ?>
                    <a href="https://youtube.com/watch?v=<?= $this->func->getYoutube($v['link_video']) ?>" title="<?= $v['name' . $this->lang] ?>"></a>
                <?php } ?>
            </div>
        <?php }
    }
     ...
     * Render video select
     */
    private function renderVideoSelect(): void
    {
        global $custom;
        $video_home = $this->photoRepo->getFeaturedVideos('video');
        ...
        if (count($video_home)) { ?>
            <div class="video-main">
                <iframe width="100%" height="100%" src="//www.youtube.com/embed/<?= $this->func->getYoutube($video_home[0]['link_video']) ?>" frameborder="0" allowfullscreen></iframe>
            </div>
            <select class="listvideos">
                <?php foreach ($video_home as $v) { ?>
                    <option value="<?= $v['id'] ?>"><?= $v['name' . $this->lang] ?></option>
                <?php } ?>
            </select>
        <?php }
    }
    ...
    /**
     * Render chi nhanh (branches)
     */
    private function renderChiNhanh(): void
    {
        global $custom;
        $chi_nhanh = $this->newsRepo->getByType('chi-nhanh', true, 0, 'numb');
                    ...
                    <div class="scroll-toa-do">
                        ...
                        <?php foreach ($chi_nhanh as $key => $value) { ?>
                            <div class="item-toa-do <?php echo $key == 0 ? "active" : "" ?>">
                                ...
                                <strong class="d-block ten-chi-nhanh"><?php echo $value['name' . $this->lang] ?> </strong>
                            </div>
                        <?php } ?>
                    </div>
                </div>
    ...
    /**
     * Render video combobox
     */
    private function renderVideoCbo(): void
    {
        global $custom;
        $videos = $this->photoRepo->getVideosByType('video');
        ...
        if (!empty($videos)) { ?>
            ...
            <select id="cbo_video">
                <?php foreach ($videos as $value) { ?>
                    <option value="<?php echo $custom->getIDyoutube($value['link_video']) ?>"><?php echo $value['name' . $this->lang] ?></option>
                <?php } ?>
            </select>
            <script>
                $(document).ready(function() {
    ...
    /**
     * Render video slick carousel
     */
    private function renderVideoSlick(): void
    {
        global $custom;
        $videos = $this->photoRepo->getFeaturedVideos('video');
    ...
    /**
     * Render Facebook messages widget
     */
    private function renderMessagesFacebook(): void
    {
        ...
        <div class="js-facebook-messenger-container">
            <div class="js-facebook-messenger-top-header">
                <span><?= $setting['name' . $this->lang] ?></span>
            </div>
    ...
    /**
     * Render main scripts (Facebook SDK, Zalo, AddThis)
     */
    private function renderScriptMain(): void
    {
        ?>
        <div id="fb-root"></div>
        <script>
