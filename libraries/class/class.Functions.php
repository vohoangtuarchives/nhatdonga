...

{
	...
	public function getSizeSC($id = 0, $name = '',$type='san-pham')

	{

		$lang = 'vi';

		$arr_size = $this->d->rawQuery("SELECT id, name$lang from table_size where type=? and find_in_set('hienthi',status) order by numb",array($type));

		$str = '';

		$str .= '<select class="select2 form-control select-size" name="' . $name . '">';
		...
		foreach ($arr_size as $v) {
			...
			$str .= '<option value="' . $v['id'] . '" ' . $select . '>' . $v['name' . $lang] . '</option>';
	...
	public function getColorSC($id = 0, $name = '',$type='san-pham')

	{

		$lang = 'vi';

		$arr_color = $this->d->rawQuery("SELECT id, name$lang from table_color where type=? and find_in_set('hienthi',status) order by numb",array($type));

		$str = '';

		$str .= '<select class="select2 form-control select-color" name="' . $name . '">';
