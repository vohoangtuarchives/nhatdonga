...
$(document).ready(function(){
	...
	$(".add-size-color").click(function () {
		...
		let key = $(".group-size-color").length;
		let type = $(this).attr("data-type");
		let target = $(this).attr("data-target");
		$.ajax({
		  url: "api/product_size_color.php",
		  type: "GET",
		  data: {
			action: "add",
			id_product: id,
		  ...
		  success: function (data) {
			...
			$(".cancel-size-color").click(function () {
			  let id_product = $(this).attr("data-id");
			  let id_size =  $(this).parents(".group-size-color").find(".select-size").length ? $(this).parents(".group-size-color").find(".select-size").val():0;
			  let id_color = $(this).parents(".group-size-color").find(".select-color").length ? $(this).parents(".group-size-color").find(".select-size").val():0;
			  $.ajax({
				url: "api/product_size_color.php",
				type: "GET",
				data: {
			  ...
			  $(this).parents(".group-size-color").remove();
			});
			// $(".save-size-color").click(function () {
			//   let id_product = $(this).attr("data-id");
			//   let id_size = $(this)
			// 	.parents(".group-size-color")
			// 	.find(".select-size")
			// 	.val();
			//   let id_color = $(this)
			// 	.parents(".group-size-color")
