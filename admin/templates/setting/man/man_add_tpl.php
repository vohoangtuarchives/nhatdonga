			...
			<div class="card-body">
				...
				<div class="row">
					...
					<?php if(isset($config['setting']['coords']) && $config['setting']['coords'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="coords">Link chỉ đường:</label>
							<input type="text" class="form-control text-sm" name="data[options][coords]" id="coords" placeholder="Link chỉ đường" value="<?=(!empty($flash->has('coords'))) ? $flash->get('coords') : @$options['coords']?>">
						</div>
					<?php } ?>
					<?php if(isset($config['setting']['worktime']) && $config['setting']['worktime'] == true) { ?>
						<div class="form-group col-md-4 col-sm-6">
							<label for="worktime">Thời gian làm việc:</label>
							<input type="text" class="form-control text-sm" name="data[options][worktime]" id="worktime" placeholder="Thời gian làm việc" value="<?=(!empty($flash->has('worktime'))) ? $flash->get('worktime') : @$options['worktime']?>">
						</div>
					<?php } ?>
				</div>
							...
							<?php foreach($config['website']['lang'] as $k => $v) { ?>
								<div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
									...
									<div class="form-group">
										...
										<input type="text" class="form-control for-seo text-sm" name="data[slogan<?=$k?>]" id="slogan<?=$k?>" placeholder="Slogan (<?=$k?>)" value="<?=(!empty($flash->has('slogan'.$k))) ? $flash->get('slogan'.$k) : @$item['slogan'.$k]?>" >
									</div>
								</div>
                            <?php } ?>
						</div>
