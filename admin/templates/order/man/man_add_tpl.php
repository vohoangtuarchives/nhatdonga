...
function getInfoSC($id, $type)
{
	global $d;
	if ((int)$id > 0) {
		if ($type) {
			$info = $d->rawQueryOne("SELECT namevi from table_$type where id=?", array($id));
			return $info['namevi'];
		}
	}
}
function getPrice($id = 0, $size = 0, $color = 0)
{
	global $d;
	$price = null;
	if ($id) {
		$rowSC = $d->rawQueryOne("select * from table_product_size_color where id_product = ? and id_color=? and id_size=? limit 0,1", array($id, $color, $size));
		if ($rowSC) {
			$price = $rowSC['price'];
		} else {
			$row = $d->rawQueryOne("select * from #_product where id = ? limit 0,1", array($id));
			if ($row['sale_price']) {
					...
					<?php } else { ?>
						<tbody>
							<?php foreach ($order_detail as $k => $v) { ?>
								<?php $priceSC = getPrice($v['id_product'], $v['size'], $v['color']) ?>
								<tr>
									...
									<td class="align-middle">
										...
										<?php if ($v['color'] || $v['size']) { ?>
											<p class="mb-0">
												<?php if ($v['color']) { ?>
													<span class="pr-2">Màu: <?= getInfoSC($v['color'], 'color') ?></span>
												<?php } ?>
												<?php if ($v['size']) { ?>
													<span>Size: <?= getInfoSC($v['size'], 'size') ?></span>
												<?php } ?>
											</p>
										<?php } ?>
									</td>
									<td class="align-middle text-center">
										<div class="price-cart-detail">
											<?php if ($priceSC) { ?>
												<span class="price-new-cart-detail"><?= $func->formatMoney($priceSC) ?></span>
											<?php } else { ?>
												<?php if ($v['sale_price']) { ?>
									...
									<td class="align-middle text-right">
										<div class="price-cart-detail">
											<?php if ($priceSC) { ?>
												<span class="price-new-cart-detail"><?= $func->formatMoney($priceSC * $v['quantity']) ?></span>
											<?php } else { ?>
												<?php if ($v['sale_price']) { ?>
