                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['tags'][$type]['desc']) && $config['tags'][$type]['desc'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['tags'][$type]['desc_cke']) && $config['tags'][$type]['desc_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[desc<?= $k ?>]" id="desc<?= $k ?>" rows="5" placeholder="Mô tả (<?= $k ?>)"><?= htmlspecialchars_decode((!empty($flash->has('desc' . $k))) ? $flash->get('desc' . $k) : (@$item['desc' . $k] ?? '')) ?></textarea>
                                                </div>
                                            <?php } ?>
                                            <?php if (isset($config['tags'][$type]['content']) && $config['tags'][$type]['content'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="content<?= $k ?>">Nội dung (<?= $k ?>):</label>
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['tags'][$type]['content_cke']) && $config['tags'][$type]['content_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="Nội dung (<?= $k ?>)"><?= htmlspecialchars_decode((!empty($flash->has('content' . $k))) ? $flash->get('content' . $k) : (@$item['content' . $k] ?? '')) ?></textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
                                    <?php } ?>
