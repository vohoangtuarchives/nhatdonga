...
<aside class="main-sidebar sidebar-dark-primary elevation-4 text-sm">
    ...
    <a class="brand-link" href="index.php">
        <img class="brand-image" src="assets/images/admin_logo.jpg" alt="Nina">
    </a>
    ...
    <div class="sidebar">
        <nav class="mt-3">
            <ul class="nav nav-pills nav-sidebar flex-column nav-child-indent text-sm" data-widget="treeview" role="menu" data-accordion="false">
                ...
                <!-- Bộ lọc -->
                <?php if (isset($config['filter']) && $config['filter']) { ?>
                    <?php
                    $none = "";
                    $active = "";
                    $menuopen = "";
                    //if(isset($kiemtra) && $kiemtra == true) if($func->check_access('filter', 'man', '', $config['filter'], 'phrase-2', false)) $none = "d-none";
                    if (($com == 'filter') && !isset($disabled['filter'][$_GET['type']]) && (!isset($config['filter'][$_GET['type']]['dropdown']) || (isset($config['filter'][$_GET['type']]['dropdown']) && $config['filter'][$_GET['type']]['dropdown'] == false))) {
                        $active = 'active';
                        $menuopen = 'menu-open';
                    }
                    ?>
                    <li class="nav-item has-treeview <?= $menuopen ?> <?= $none ?>">
                        <a class="nav-link <?= $active ?>" href="#" title="Quản lý bộ lọc">
                        ...
                        <ul class="nav nav-treeview">
                            <?php foreach ($config['filter'] as $k => $v) {
                                if (!isset($disabled['filter'][$k]) && (!isset($v['dropdown']) || (isset($v['dropdown']) && $v['dropdown'] == false))) { ?>
                                    <?php
                                    $none = "";
                                    $active = "";
                                    if (isset($kiemtra) && $kiemtra == true) if ($func->check_access('filter', 'man', $k, null, 'phrase-1')) $none = "d-none";
                                    if ($com == 'filter' && ($act == 'man' || $act == 'add' || $act == 'edit' || $act == 'copy' || $kind == 'man') && $k == $_GET['type']) $active = "active";
                                    ?>
                                    <li class="nav-item <?= $none ?>">
                                        <a class="nav-link <?= $active ?>" href="index.php?com=filter&act=man&type=<?= $k ?>" title="<?= $v['title_main'] ?>"><i class="nav-icon text-sm far fa-caret-square-right"></i>
                                            <p><?= $v['title_main'] ?></p>
                                        </a>
                                    </li>
                            <?php }
