    ...
    <form class="validation-form" novalidate method="post" action="<?= $linkSave ?>" enctype="multipart/form-data">
        ...
        <div class="row">
                                        ...
                                        <div class="tab-pane fade show <?= ($k == 'vi') ? 'active' : '' ?>" id="tabs-lang-<?= $k ?>" role="tabpanel" aria-labelledby="tabs-lang">
                                            ...
                                            <?php if (isset($config['news'][$type]['desc']) && $config['news'][$type]['desc'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea
                                                            class="form-control for-seo text-sm <?= (isset($config['news'][$type]['desc_cke']) && $config['news'][$type]['desc_cke'] == true) ?
                                                        'form-control-ckeditor' : '' ?>"
                                                              name="data[desc<?= $k ?>]"
                                                              id="desc<?= $k ?>"
                                                            rows="5"
                                                            placeholder="Mô tả (<?= $k ?>)"><?= $func->htmlspecialchars_decode((!empty($flash->has('desc' . $k))) ? $flash->get('desc' . $k) : ($item['desc' . $k] ?? '')) ?></textarea>
                                                </div>
                                            <?php } ?>

                                            <?php if (isset($config['news'][$type]['career']) && $config['news'][$type]['career'] == true) { ?>
                                                <div class="form-group">
                                                    <label for="desc<?= $k ?>">Nghề nghiệp (<?= $k ?>):</label>
                                                    <select name="data[career]" id="career" class="form-control">
                                                        <?php foreach($config['careers'] as $key => $value){ ?>
                                                            <option value="<?=$key?>" <?php if(isset($item['career']) && $item['career'] == $key) echo("selected");?>><?=$value?></option>
                                                        <?php } ?>
                                                    </select>
                                                </div>
                                            <?php } ?>
                                            ...
                                            <?php if (isset($config['news'][$type]['content']) && $config['news'][$type]['content'] == true) { ?>
                                                <div class="form-group">
                                                    ...
                                                    <textarea class="form-control for-seo text-sm <?= (isset($config['news'][$type]['content_cke']) && $config['news'][$type]['content_cke'] == true) ? 'form-control-ckeditor' : '' ?>" name="data[content<?= $k ?>]" id="content<?= $k ?>" rows="5" placeholder="Nội dung (<?= $k ?>)">
                                                        <?= $func->htmlspecialchars_decode((!empty($flash->has('content' . $k))) ? $flash->get('content' . $k) : (@$item['content' . $k] ?? '')) ?>
                                                    </textarea>
                                                </div>
                                            <?php } ?>
                                        </div>
            ...
            <div class="<?= $colRight ?>">
                ...
                <?php if (isset($config['news'][$type]['images2']) && $config['news'][$type]['images2'] == true) { ?>
					<div class="card card-primary card-outline text-sm">
						...
						<div class="card-body">
							...
							$photoDetail['dimension'] = "Width: " . $config['news'][$type]['width2'] . " px - Height: " . $config['news'][$type]['height2'] . " px (" . $config['news'][$type]['img_type'] . ")";
				...
				<?php } ?>
				<?php if (isset($config['news'][$type]['images3']) && $config['news'][$type]['images3'] == true) { ?>
					<div class="card card-primary card-outline text-sm">
						<div class="card-header">
							<h3 class="card-title">Hình ảnh 3 <?= $config['news'][$type]['title_main'] ?></h3>
							<div class="card-tools">
								<button type="button" class="btn btn-tool" data-card-widget="collapse"><i class="fas fa-minus"></i></button>
							</div>
						</div>
						<div class="card-body">
							...
							$photoDetail['dimension'] = "Width: " . $config['news'][$type]['width3'] . " px - Height: " . $config['news'][$type]['height3'] . " px (" . $config['news'][$type]['img_type'] . ")";
            ...
            <div class="card-body">
                ...
                <?php if (isset($config['news'][$type]['iframe']) && $config['news'][$type]['iframe'] == true) { ?>
                    <div class="form-group">
                        ...
                        <textarea class="form-control  text-sm" name="data[iframe]" id="iframe" rows="5" placeholder="Iframe"><?= !empty($item['iframe']) ? $func->htmlspecialchars_decode($item['iframe']) : "" ?></textarea>
                    </div>
                <?php } ?>
                <?php if (isset($config['news'][$type]['link']) && $config['news'][$type]['link'] == true) { ?>
                    <div class="form-group">
                        <label for="link">Link:</label>
                        <input class="form-control  text-sm" name="data[link]" id="link" placeholder="Link" value="<?= !empty($item['link']) ? $item['link'] : "" ?>">
                    </div>
                <?php } ?>
            </div>
        </div>
        ...
        <?php if (isset($config['news'][$type]['schema']) && $config['news'][$type]['schema'] == true) { ?>
            <div class="card card-primary card-outline text-sm">
                <div class="card-header">
                    <h3 class="card-title">Schema JSON Article <a href="https://developers.google.com/search/docs/advanced/structured-data/search-gallery">(Tài liệu tham khảo)</a></h3>
