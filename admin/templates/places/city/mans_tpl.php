...
$linkAdd = "index.php?com=places&act=add_city";
$linkEdit = "index.php?com=places&act=edit_city";
$linkDelete = "index.php?com=places&act=delete_city";
?>
<!-- Content Header -->
            ...
            <table class="table table-hover">
                <thead>
                    <tr>
                        ...
                        <?php if (isset($config['places']['ship_price']) && in_array("city", $config['places']['ship_price'])) { ?>
                    ...
                    <tbody>
                        <?php for ($i = 0; $i < count($items); $i++) { ?>
                            <tr>
                                ...
                                <td class="align-middle">
                                    <input type="number" class="form-control form-control-mini m-auto update-numb" min="0" value="<?= $items[$i]['numb'] ?>" data-id="<?= $items[$i]['id'] ?>" data-table="city">
                                </td>
                                <td class="align-middle">
                                ...
                                <?php if (isset($config['places']['check_city'])) {
                                    foreach ($config['places']['check_city'] as $key => $value) { ?>
                                        <td class="align-middle text-center">
                                            <div class="custom-control custom-checkbox my-checkbox">
                                                <input type="checkbox" class="custom-control-input show-checkbox" id="show-checkbox-<?= $key ?>-<?= $items[$i]['id'] ?>" data-table="city" data-id="<?= $items[$i]['id'] ?>" data-attr="<?= $key ?>" <?= (in_array($key, $status_array)) ? 'checked' : '' ?>>
