...
function saveMan()
{
	...
	$dataSC = (!empty($_POST['dataSC'])) ? $_POST['dataSC'] : null;
	...
	$dataFilter = (!empty($_POST['dataFilter'])) ? $_POST['dataFilter'] : null;
	...
	if ($id && $act != 'save_copy') {
		...
		if ($d->update('product', $data)) {
			...
			/*Filter*/
			if(isset($config['product'][$type]['filter']) && count($config['product'][$type]['filter']) > 0){
				if($dataFilter){
					$d->rawQuery("DELETE from table_filter_relation where id_parents=? and com='product' and type=?",array($id,$type));
					foreach($dataFilter as $v){
						$dataMultiFilter = array();
						$dataMultiFilter['com']='product';
						$dataMultiFilter['type']=$type;
						$dataMultiFilter['id_parents']=$id;
						$dataMultiFilter['id_filter']=$v;
						$d->insert('filter_relation', $dataMultiFilter);
					}
				}
			}
			...
			if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size'])) {
				if (isset($config['product'][$type]['price_size_color']) && count($config['product'][$type]['price_size_color'])>0) {
					//save giá theo size,màu
					foreach ($dataSC as $v) {
						$dataSCMulti = array();
						if(in_array("size",$config['product'][$type]['price_size_color'])){
							$dataSCMulti['id_size'] = (int)$v['size'];
						} else {
							$dataSCMulti['id_size'] = 0;
						}
						if(in_array("color",$config['product'][$type]['price_size_color'])){
							$dataSCMulti['id_color'] = (int)$v['color'];
						} else {
							$dataSCMulti['id_color'] = 0;
						}
						$dataSCMulti['price'] = str_replace(",", "", $v['price']);
						$dataSCMulti['id_product'] = $id;
						$id_SCMulti = (int)$v['id'];
						if ($dataSCMulti['price']) {
							if ($id_SCMulti) {
								$rowSC = $d->rawQueryOne("SELECT * from table_product_size_color where id=?", array($id_SCMulti));
								if ($rowSC) {
									$d->where('id', $id_SCMulti);
									$d->update('product_size_color', $dataSCMulti);
								}
							} else {
								$rowSC = $d->rawQueryOne("SELECT * from table_product_size_color where id_product=? and id_color=? and id_size=?", array($dataSCMulti['id_product'], $dataSCMulti['id_color'], $dataSCMulti['id_size']));
								if ($rowSC) {
									$d->where('id', $rowSC['id']);
									$d->update('product_size_color', $dataSCMulti);
								} else {
									$d->insert('product_size_color', $dataSCMulti);
								}
							}
							//save product_sale
							// $d->rawQuery("delete from #_product_sale where id_parent = ?", array($id));
							$dataSaleSC = array();
							$dataSaleSC['id_parent'] = $id;
							$dataSaleSC['id_color'] = $dataSCMulti['id_color'];
							$dataSaleSC['id_size'] = $dataSCMulti['id_size'];
							$rowSaleSC = $d->rawQuery("SELECT * from table_product_sale where id_parent=? and id_color=? and id_size=?", array($dataSaleSC['id_parent'], $dataSaleSC['id_color'], $dataSaleSC['id_size']));
							if (empty($rowSaleSC)) {
								$d->insert('product_sale', $dataSaleSC);
							}
						}
					}
				} else {
			...
			/* Photo */
			if ($func->hasFile("file3")) {
				$photoUpdate3 = array();
				$file_name3 = $func->uploadName($_FILES["file3"]["name"]);

				if ($photo3 = $func->uploadImage("file3", $config['product'][$type]['img_type'], UPLOAD_PRODUCT, $file_name3)) {
					$row = $d->rawQueryOne("select id, photo3 from #_product where id = ? and type = ? limit 0,1", array($id, $type));

					if (!empty($row)) {
						$func->deleteFile(UPLOAD_PRODUCT . $row['photo3']);
					}

					$photoUpdate3['photo3'] = $photo3;
					$d->where('id', $id);
					$d->update('product', $photoUpdate3);
					unset($photoUpdate3);
				}
			}
			...
			if (isset($config['product'][$type]['schema']) && $config['product'][$type]['schema'] == true) {
				...
				if ($buildSchema) {
					foreach ($config['website']['seo'] as $k => $v) {
						...
						$dataSchema['schema' . $k] = $func->buildSchemaProduct($id, $data['name' . $k], $url_img_pro, $dataSeo['description' . $k], $data['code'], $pro_list['ten'], $setting['name' . $k], $url_pro, $data['sale_price']);
					}
				} else {
			...
			if(isset($config['product'][$type]['filter']) && count($config['product'][$type]['filter']) > 0){
				if($dataFilter){
					foreach($dataFilter as $v){
						...
						$dataMultiFilter['com']='product';
						$dataMultiFilter['type']=$type;
						$dataMultiFilter['id_parents']=$id_insert;
						$dataMultiFilter['id_filter']=$v;
						$d->insert('filter_relation', $dataMultiFilter);
					}
				}
			...
			if (!empty($config['product'][$type]['color']) || !empty($config['product'][$type]['size'])) {
				if (isset($config['product'][$type]['price_size_color']) && count($config['product'][$type]['price_size_color'])>0) {
					...
					foreach ($dataSC as $v) {
						...
						$dataSCMulti['id_color'] = (int)$v['color'];
						$dataSCMulti['price'] = str_replace(",", "", $v['price']);
						$dataSCMulti['id_product'] = $id_insert;
						if ($dataSCMulti['price']) {
							$rowSC = $d->rawQueryOne("SELECT * from table_product_size_color where id_product=? and id_color=? and id_size=?", array($dataSCMulti['id_product'], $dataSCMulti['id_color'], $dataSCMulti['id_size']));
							if ($rowSC) {
								$d->where('id', $rowSC['id']);
							...
							//save product_sale
							$dataSaleSC = array();
							$dataSaleSC['id_parent'] = $id_insert;
							$dataSaleSC['id_color'] = $dataSCMulti['id_color'];
							$dataSaleSC['id_size'] = $dataSCMulti['id_size'];
							$rowSaleSC = $d->rawQuery("SELECT * from table_product_sale where id_parent=? and id_color=? and id_size=?", array($dataSaleSC['id_parent'], $dataSaleSC['id_color'], $dataSaleSC['id_size']));
							if ($rowSaleSC) {
								$d->where('id', $rowSC['id']);
								$d->update('product_sale', $dataSaleSC);
							} else {
								$d->insert('product_sale', $dataSaleSC);
							}
						}
			...
			if (isset($config['product'][$type]['schema']) && $config['product'][$type]['schema'] == true) {
				...
				if ($buildSchema) {
					foreach ($config['website']['seo'] as $k => $v) {
						...
						//Tiến hành build schema product
						$dataSchema['schema' . $k] = $func->buildSchemaProduct($id_insert, $data['name' . $k], $configBase . UPLOAD_PRODUCT_L . $data['image'], $dataSeo['description' . $k], $data['code'], $pro_list['ten'], $setting['name' . $k], $url_pro, $data['sale_price']);
					}
				} else {
					$dataSchema = (isset($_POST['dataSchema'])) ? $_POST['dataSchema'] : null;
					if ($dataSchema) {
...
function deleteMan()
{
	...
	if ($id) {
		...
		if (!empty($row)) {
			...
			/*xóa price-size*/
			$d->rawQuery("delete from table_product_size_color where id_product = ?", array($id));
...
function deleteSize()
{
	...
	if ($id) {
		...
		if (!empty($row)) {
			...
			/*xóa price-size*/
			$d->rawQuery("delete from table_product_size_color where id_size = ?", array($id));
...
function deleteColor()
{
	...
	if ($id) {
		...
		if (!empty($row)) {
			...
			/*xóa price-size*/
			$d->rawQuery("delete from table_product_size_color where id_color = ?", array($id));
